...
#on doit aussu definir un comportement avance 

class Comportementav(Comportement):
    RUN_COEF = maxPlayerAcceleration
    GO_COEF = maxPlayerAcceleration/3.
    SHOOT_COEF = maxPlayerShoot/3.
    THROW_COEF = maxPlayerShoot
    def __init__(self,state):
        super(Comportementav,self).__init__(state)
    def run(self,p):
...
class ConditionSmart(ProxyObj):
    ...
    def close_goal(self):
        return self.me.distance(self.his_goal)<self.COEF_SHOOT*self.width
    def close_ball(self):
        return self.me.distance(self.ball_p)<self.COEF_BALL*self.width #ewve houn smat  
...
def smart(I):
   if not I.can_kick:
        if I.close_ball():
            return I.run(I.ball_p) #3agbne vem 
        else:
            return I.run(I.ball_p)
   else:
        if I.close_goal():
