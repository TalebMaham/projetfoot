...
def my_get_features(state,idt,idp):
    ...
    state = SuperState(state,idt,idp)# state obejt  de class SuperState 
    f1 = state.distance(state.ball_p)# distance est une methode de superState
    f2 = state.distance(state.my_goal)
    f3 = state.ball_p.distance(state.my_goal)
    return [f1,f2,f3]
#ici on trouve quelque informations 
...
def entrainer(fname):
    ...
    #ou est le fichier à sauvegarder ?
...
# ici on doit ajouter dans cette liste des strategies 
    dic_strategy = {FonceurStrategy().name:FonceurStrategy(),DefenseurStrategy().name:DefenseurStrategy(),SmartStrategy().name:SmartStrategy()}
